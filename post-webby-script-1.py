# Filename: webinarjam_hot_leads_processor.py
import os
import requests
import time
import logging
...
# --- Setup Logger ---
log_file_name = 'webinarjam_hot_leads.log' # Specific log file
...
logger.info("Script started (Hot Leads Processor). Attempting to load environment variables...")
...
def determine_hot_lead(registrant):
    ...
    return 1 if total_seconds >= 7200 else 0 # 2 hours for hot lead
...
# --- Main Processing Logic for Hot Leads & High Engagement ---
def process_high_engagement_hot_leads():
    logger.info(f"Starting processing for HIGH ENGAGEMENT & HOT LEADS: Webinar '{WEBINAR_ID}', Schedule '{WEBINAR_SCHEDULE_ID}'.")
    page = 1
    processed_count = 0
    api_fetched_count = 0
    ...
    while True:
        api_payload = {
            ...
            "date_range": 0,
            "attended_live": 1, # IMPORTANT: Fetch only those who attended
            "page": page
        }
        logger.debug(f"Fetching attended registrants (Page {page}): {api_payload}")
        try:
            ...
            if response_json.get("status") == "success":
                ...
                if not data and page == 1: logger.info("No attended registrants found."); break
                if not data: logger.info(f"No more attended registrants. Last page: {page-1}."); break
                ...
                for registrant in data:
                    ...
                    tag = determine_tag(registrant) # Will be 'high engagement' or 'low engagement'
                    is_hot_lead = determine_hot_lead(registrant)

                    if tag == "high engagement" and is_hot_lead == 1:
                        logger.info(f"MATCH: {email} is High Engagement AND Hot Lead. Sending to GHL.")
                        purchased = 1 if str(registrant.get("purchased_live", "")).lower() == "yes" else 0
                        send_to_ghl(registrant, tag, purchased, is_hot_lead)
                        processed_count += 1
                    # else:
                        # logger.debug(f"NO MATCH: {email}. Tag: {tag}, Hot Lead: {is_hot_lead}. Not sending for this script.")
                
                if current_page_api >= total_pages_api or not data:
                    logger.info(f"End of attended list. Current API page: {current_page_api}, Total: {total_pages_api}."); break
                page += 1
    ...
    logger.info(f"High Engagement & Hot Leads processing finished. API Fetched: {api_fetched_count}, Sent to GHL: {processed_count}.")

# --- Main Execution ---
if __name__ == "__main__":
    logger.info("--- High Engagement & Hot Leads Processor Script Started ---")
    process_high_engagement_hot_leads()
    logger.info("--- High Engagement & Hot Leads Processor Script Finished ---")